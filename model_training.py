...
def train_and_evaluate(
    ...
    FEATURE_COLUMNS_FOR_TRAINING = [
        ...
        if col
        not in ["Date", "Target_Tomorrow", "Target_3_Days", "Target_Next_Week", "Close"]
    ]
    ...
    X_backtest, y_backtest = (
        backtest_df[FEATURE_COLUMNS_FOR_TRAINING],
        backtest_df[TARGET_COLUMNS],
    )
    ...
    X_test_tensor = torch.tensor(X_test, dtype=torch.float32).view(
        -1, 1, X_test.shape[1]
    )
    y_test_tensor = torch.tensor(y_test.values, dtype=torch.float32)

    X_backtest_tensor = torch.tensor(X_backtest, dtype=torch.float32).view(
        -1, 1, X_backtest.shape[1]
    )
    ...
    model = get_model(
        input_size=len(FEATURE_COLUMNS_FOR_TRAINING),
