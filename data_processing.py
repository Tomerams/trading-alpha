...
def get_data(stock_ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    exclude_cols = ["Date", "Target_Tomorrow", "Target_3_Days", "Target_Next_Week", "Close"]
...
def calculate_indicators(df: pd.DataFrame, add_external_features: bool = True) -> pd.DataFrame:
...
def calculate_gann_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[GannFeaturs.UP_CYCLE_LENGTH.value], df[GannFeaturs.DOWN_CYCLE_LENGTH.value] = get_cycle_times(df)
    return df
...
def calculate_fibonacci_levels(df: pd.DataFrame, window: int = 20) -> pd.DataFrame:
    ...
    df[GannFeaturs.FIB_23_ABOVE.value] = (df["Close"] > (highest - 0.236 * range_)).astype(int)
    df[GannFeaturs.FIB_50_ABOVE.value] = (df["Close"] > (highest - 0.5 * range_)).astype(int)
    return df
...
def add_external_market_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def download(ticker, colname):
        ext = yf.download(ticker, start=df["Date"].min(), end=df["Date"].max(), interval="1d")
        ext = ext.copy()
    ...
    df[ExternalFeatures.SPY_VIX_RATIO.value] = df[ExternalFeatures.SPY_CLOSE.value] / df[ExternalFeatures.VIX_CLOSE.value]
    df[ExternalFeatures.YIELD_SPREAD_10Y_2Y.value] = df[ExternalFeatures.US10Y_YIELD.value] - df[ExternalFeatures.US2Y_YIELD.value]
    return df
