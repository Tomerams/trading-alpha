...
def generate_swing_signals(
    close: pd.Series, window: int = 5, prominence: float = 0.01
) -> pd.Series:
    ...
    events = sorted(
        [(i, "buy") for i in troughs] + [(i, "sell") for i in peaks], key=lambda x: x[0]
    )
    state = "buy"
    trades = []
...
def add_swing_column(
    ...
    signals = generate_swing_signals(
        df[close_col], window=window, prominence=prominence
    )
