...
# ────────────────────────────────────────────────────────────────────
def _next_peaks_troughs(
    close: np.ndarray, prominence: float
...
def make_action_label_swing(
    ...
    # ── derive action ──
    action = np.where(delta_up >= up_thr, 2, np.where(delta_dn <= dn_thr, 0, 1)).astype(
        int
    )
...
def _next_pk_tr(price: np.ndarray, prom: float):
    ...
    nxt_pk = np.full_like(price, np.nan)
    nxt_tr = np.full_like(price, np.nan)
    p_iter, t_iter = iter(pk[::-1]), iter(tr[::-1])
    p, t = next(p_iter, None), next(t_iter, None)
...
def make_action_label_clean(
    df: pd.DataFrame,
    up_thr: float = 0.015,
    dn_thr: float = -0.015,
    atr_mult: float = 1.0,
    atr_col: str = "ATR",
    margin_thr: float | None = 0.005,  # ← תוספת היחידה
) -> np.ndarray:
    price = df["Close"].to_numpy(float)
    atr = df[atr_col].ffill().to_numpy(float)
    prom = atr_mult * np.nanmedian(atr)  # “פיק” משמעותי

    # חישוב שיאים ותחתיות עתידיים
    ...
    lbl = np.where(
        delta_up >= up_thr, 2, np.where(delta_dn <= dn_thr, 0, 1)  # BUY  # SELL / HOLD
    ).astype(
        float
    )  # float לצורך NaN

    # ───── סינון רעש ע״י margin_thr ─────
    if margin_thr is not None:
        r1d = df["Close"].pct_change().shift(-1).to_numpy(float)
        lbl[np.abs(r1d) < margin_thr] = np.nan  # תצפיות “זניחות” → drop
